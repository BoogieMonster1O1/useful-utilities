...
@Environment(EnvType.CLIENT) class UtilitiesListDescription() extends LightweightGuiDescription {
...

	{
		...
		calc.setOnClick(() => MinecraftClient.getInstance.setScreen(new UtilityScreen(new CalculatorDescription)))
		val cps = new WButton(new TranslatableText("gui.utilities.cps"))
		cps.setEnabled(true)
		root.add(cps, 7, 3, 6, 1)
		cps.setOnClick(() => MinecraftClient.getInstance.setScreen(new UtilityScreen(new ClicksPerSecondDescription)))
		val portal = new WButton(new TranslatableText("gui.utilities.portal"))
		portal.setEnabled(true)
		root.add(portal, 14, 3, 6, 1)
		portal.setOnClick(() => MinecraftClient.getInstance.setScreen(new UtilityScreen(new PortalCoordinatesDescription)))
		val random = new WButton(new TranslatableText("gui.utilities.random"))
		random.setEnabled(true)
		root.add(random, 0, 5, 6, 1)
		random.setOnClick(() => MinecraftClient.getInstance.setScreen(new UtilityScreen(new RandomNumberDescription)))
		val alarm = new WButton(new TranslatableText("gui.utilities.alarm"))
		alarm.setEnabled(true)
		alarm.setOnClick(() => MinecraftClient.getInstance.setScreen(new UtilityScreen(new AlarmDescription)))
		root.add(alarm, 7, 5, 6, 1)
		val crash = new WButton(new TranslatableText("gui.utilities.crash"))
		crash.setEnabled(true)
		crash.setOnClick(() => MinecraftClient.getInstance.setScreen(new UtilityScreen(new GameCrashersDescription)))
		...
		worldseed.setOnClick(() => MinecraftClient.getInstance.setScreen(new UtilityScreen(new WorldInfoDescription)))
		val keyboard = new WButton(new TranslatableText("gui.utilities.keyboard"))
		keyboard.setEnabled(true)
		root.add(keyboard, 7, 7, 6, 1)
		keyboard.setOnClick(() => MinecraftClient.getInstance.setScreen(new PianoDescription.KeyboardMainScreen))
		...
		expense.setOnClick(() => {
			def foo() = {
				...
				if (!initFIle.exists) expense.setOnClick(() => {
					try MinecraftClient.getInstance.setScreen(new ExpenseScreen)
					catch {
						case exemption: IOException =>
							exemption.printStackTrace()
					}
				})
				else try MinecraftClient.getInstance.setScreen(new UtilityScreen(new TranslatableText("gui.utilities.expense"), new ExpenseDescription(true)))
				catch {
		...
		arraySort.setOnClick(() => MinecraftClient.getInstance.setScreen(new UtilityScreen(new TranslatableText(""), new ArraySortDescription)))
		...
		httpServer.setOnClick(() => {
			try MinecraftClient.getInstance.setScreen(new UtilityScreen(new TranslatableText("gui.utilities.monsterhttp"), new HttpServerDescription))
			catch {
				case e: Throwable =>
					e.printStackTrace()
			}
